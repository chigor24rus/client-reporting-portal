...
type FormMode = 'add' | 'password';

export default function AdminsPage() {
  const { admins, user, addAdmin, removeAdmin, changeAdminPassword } = useApp();
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    const ok = addAdmin({ name: form.name.trim(), phone: form.phone, password: form.password });
    if (!ok) return setError('Администратор с таким номером уже существует');
  ...
  function handlePasswordChange(e: React.FormEvent) {
    ...
    changeAdminPassword(editId, form.password);
    ...
    <div className="p-6 space-y-6 max-w-2xl">
        ...
        <div className="bg-card border border-primary/30 rounded-xl p-5 animate-fade-in">
          ...
          <form onSubmit={formMode === 'add' ? handleAdd : handlePasswordChange} className="space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button type="button" onClick={resetForm} className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground border border-border rounded-lg transition-colors">
                Отмена
              </button>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {isSelf(admin.id) && (
                    <span className="text-xs px-2 py-0.5 rounded bg-primary/15 text-primary font-medium">Это вы</span>
                  )}
                </div>
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  <span className="font-mono">{admin.phone}</span>
                  <span>·</span>
                  <span>Добавлен {new Date(admin.createdAt).toLocaleDateString('ru-RU')}</span>
                </div>
              </div>

              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  onClick={() => openPasswordEdit(admin.id)}
                ...
                {!isSelf(admin.id) && (
                  deleteId === admin.id ? (
                    <div className="flex gap-1">
                      <button
                        onClick={() => { removeAdmin(admin.id); setDeleteId(null); }}
      ...
      <div className="bg-card border border-border rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-xs text-muted-foreground">
          Администраторы имеют полный доступ ко всем разделам портала: загрузке отчётов, управлению мастерами, статистике и отчётам. Себя удалить нельзя.
        </p>
      </div>
    </div>
  );
